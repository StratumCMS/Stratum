        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST">
            ...
            <div class="space-y-6">
                    ...
                    <div class="p-6 pt-0 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="title" class="w-full h-10 px-3 rounded-md border border-border bg-background text-foreground placeholder:text-muted-foreground shadow-sm focus:outline-none focus:ring-2 focus:ring-primary transition"
                            ...
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" class="w-full h-10 px-3 rounded-md border border-border bg-background text-foreground placeholder:text-muted-foreground shadow-sm focus:outline-none focus:ring-2 focus:ring-primary transition"
                                       required value="{{ old('slug', $page->slug) }}">
                        ...
                        <div class="flex items-center justify-between border rounded p-4">
                            ...
                            <label class="switch">
                                <input type="checkbox" name="is_home" value="1" @checked(old('is_home', $page->is_home)) class="h-4 w-4 text-primary bg-background border-border rounded focus:ring-primary focus:ring-2 focus:ring-offset-0 transition">
                    ...
                    <div class="p-6 pt-0 space-y-4">
                        <div>
                            ...
                            <textarea name="meta_description" class="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                      rows="4" placeholder="Description pour les moteurs de recherche (max 160 caractères)" required>{{ old('meta_description', $page->meta_description) }}</textarea>
                ...
                <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
                    ...
                    <div class="p-6 pt-0 space-y-4">
                        <textarea name="content" id="tinymce" class="tinymce" required>{{ old('content', $page->content) }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-primary-foreground text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 bg-primary hover:bg-primary/90 hover-glow-purple h-11 px-8">
                        <i class="fas fa-save h-4 w-4 mr-2"></i> Modifier la page
                    </button>
                </div>
